...
import { IngredientTypes } from "../../types/dataTypes";
  ...
  describe("Проверка selectedIngredientsSlice", () => {
    ...
    it("addSelectedIngredient добавляет ингредиентам uniqueId", async () => {
      ...
      expect(firstInnerIngredient.uniqueId).not.toEqual(secondInnerIngredient.uniqueId);
    });
    ...
    it("removeSelectedIngredient удаляет ингредиент по uniqueId", async () => {
      ...
      expect(getState().inner).toMatchObject([
        firstMain,
        secondMain,
      ]);
    });

    it("reorderSelectedIngredients при перемещении вниз "
    ...
    + "под целевым", async () => {
      ...
      store.dispatch(reorderSelectedIngredients({
    ...
    it("reorderSelectedIngredients при перемещении вверх "
    + "помещает перемещаемый ингредиент "
    + "над целевым", async () => {
      const { getState } = store;
